  ...
  return <footer className="border-t border-border bg-surface-raised">
    <div className="container-shell py-16">
      <div className="grid gap-12 lg:grid-cols-[1.3fr_.7fr_.7fr_1.2fr]">
        ...
        <div><h3 className="text-sm font-bold">Useful ideas, no noise.</h3><p className="mt-3 text-sm leading-6 text-muted">A monthly field note on digital growth and resilient operations.</p><form onSubmit={subscribe} className="mt-5 flex gap-2"><input required type="email" className="input min-w-0" placeholder="you@company.com" value={email} onChange={(event) => setEmail(event.target.value)} /><button disabled={subscribing} className="grid size-12 shrink-0 place-items-center rounded-xl bg-primary text-primary-foreground transition hover:bg-primary/90 disabled:opacity-60"><Send size={17} /></button></form></div>
      </div>
