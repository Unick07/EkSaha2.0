...
export default function AppShell({ variant = "user" }) {
  ...
  return <div className="min-h-screen bg-background text-text">
    ...
    <div className={`transition-all duration-300 ${collapsed ? "lg:pl-20" : "lg:pl-72"}`}><header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-border bg-surface/85 px-5 backdrop-blur sm:px-8"><div className="flex items-center gap-3"><button className="icon-button lg:hidden" onClick={() => setSidebarOpen(true)}><Menu/></button><button title={collapsed ? "Show side panel" : "Hide side panel"} className="icon-button hidden lg:grid" onClick={toggleSidebarCollapsed}>{collapsed ? <PanelLeftOpen size={19}/> : <PanelLeftClose size={19}/>}</button><div><h1 className="text-lg font-bold">{title}</h1><p className="hidden text-xs text-muted sm:block">{isStaff ? subtitleByVariant[variant] : "Welcome back, " + user.name.split(" ")[0]}</p></div></div><div className="flex items-center gap-2"><button title="Toggle theme" onClick={toggleTheme} className="icon-button">{dark ? <Sun size={18}/> : <Moon size={18}/>}</button><button title="Notifications" onClick={() => setNotificationsOpen(true)} className="icon-button relative"><Bell size={18} className={unread > 0 ? "animate-pulse" : ""}/>{unread > 0 && <span className="absolute right-1.5 top-1.5 grid size-4 animate-pulse place-items-center rounded-full bg-primary text-[9px] font-bold text-primary-foreground">{unread > 9 ? "9+" : unread}</span>}</button></div></header><main className="p-5 sm:p-8"><Outlet/></main></div>
