...
export default function AppShell({ variant = "user" }) {
  ...
  const { theme, toggleTheme, sidebarOpen, setSidebarOpen, sidebarCollapsed, toggleSidebarCollapsed } = useAppStore();
  ...
  return <div className="min-h-screen bg-background text-text">
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex transform flex-col border-r border-border bg-surface p-4 text-text shadow-2xl transition-all duration-300 lg:translate-x-0 ${collapsed ? "lg:w-20" : "w-72"} ${sidebarOpen ? "translate-x-0 w-72" : "-translate-x-full"}`}>
      ...
      <div className="flex-shrink-0 border-t border-border pt-4">{!collapsed && <div className="mb-3 flex items-center gap-3 rounded-xl border border-border bg-surface-raised p-3"><span className="grid size-9 place-items-center rounded-full bg-gradient-to-br from-brand-navy to-brand-teal-deep text-sm font-bold text-white">{user.name[0]}</span><div className="min-w-0 flex-1"><div className="truncate text-sm font-bold">{user.name}</div><div className="truncate text-xs text-muted">{user.email}</div></div></div>}<button title={collapsed ? "Sign out" : undefined} onClick={logout} className={`flex w-full items-center rounded-xl py-3 text-sm font-semibold text-muted transition hover:bg-surface-raised hover:text-text ${collapsed ? "justify-center px-2" : "gap-3 px-4"}`}><LogOut size={17}/>{!collapsed && "Sign out"}</button></div>
    </aside>
    <div className={`transition-all duration-300 ${collapsed ? "lg:pl-20" : "lg:pl-72"}`}><header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-border bg-surface/85 px-5 backdrop-blur sm:px-8"><div className="flex min-w-0 items-center gap-3"><button className="icon-button shrink-0 lg:hidden" onClick={() => setSidebarOpen(true)}><Menu/></button><button title={collapsed ? "Show side panel" : "Hide side panel"} className="icon-button hidden shrink-0 lg:grid" onClick={toggleSidebarCollapsed}>{collapsed ? <PanelLeftOpen size={19}/> : <PanelLeftClose size={19}/>}</button><div className="min-w-0"><h1 className="truncate text-lg font-bold">{title}</h1><p className="hidden truncate text-xs text-muted sm:block">{isStaff ? subtitleByVariant[variant] : "Welcome back, " + user.name.split(" ")[0]}</p></div></div><div className="flex shrink-0 items-center gap-2">{headerAction}<button title="Toggle theme" onClick={toggleTheme} className="icon-button">{dark ? <Sun size={18}/> : <Moon size={18}/>}</button><button title="Notifications" onClick={() => setNotificationsOpen(true)} className="icon-button relative"><Bell size={18} className={unread > 0 ? "animate-pulse" : ""}/>{unread > 0 && <span className="absolute right-1.5 top-1.5 grid size-4 animate-pulse place-items-center rounded-full bg-primary text-[9px] font-bold text-primary-foreground">{unread > 9 ? "9+" : unread}</span>}</button></div></header><main className="p-5 sm:p-8"><Outlet context={{ setHeaderAction }}/></main></div>
