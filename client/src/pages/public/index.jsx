...
export function Pricing() {
  ...
  return <><section className="bg-ink py-24 text-center text-white"><div className="container-shell"><span className="eyebrow">Pricing</span><h1 className="text-5xl font-extrabold tracking-[-.05em]">A better team. A simpler bill.</h1><p className="mx-auto mt-5 max-w-2xl text-slate-300">No setup fees, hidden markups or long contracts. Just clear capacity and an experienced team.</p><div className="mx-auto mt-8 flex w-fit rounded-xl bg-white/10 p-1">{["monthly", "yearly"].map(item => <button key={item} onClick={() => setBilling(item)} className={`rounded-lg px-5 py-2 text-sm font-bold capitalize ${billing === item ? "bg-white text-ink" : "text-slate-300"}`}>{item}</button>)}</div></div></section><section className="py-20"><div className="container-shell"><div className="grid gap-6 lg:grid-cols-3">{plans.map(plan => <PlanCard plan={plan} billing={billing} key={plan.name} />)}</div><div className="mt-20 overflow-x-auto rounded-3xl border border-slate-200 dark:border-white/10"><table className="w-full min-w-[700px] text-left text-sm"><thead className="bg-slate-50 dark:bg-white/5"><tr><th className="p-5">Compare plans</th>{plans.map(p => <th className="p-5" key={p.name}>{p.name}</th>)}</tr></thead><tbody>{allFeatures.map((feature, i) => <tr className="border-t border-slate-100 dark:border-white/10" key={feature}><td className="p-5 font-semibold">{feature}</td>{plans.map((p, pi) => <td className="p-5 text-slate-500" key={p.name}>{i === 0 ? (pi === 0 ? "1 service" : pi === 1 ? "3 services" : "All services") : i === 1 ? (pi === 0 ? "10 / month" : "Unlimited") : <Check size={17} className="text-emerald-500" />}</td>)}</tr>)}</tbody></table></div></div></section></>;
}
