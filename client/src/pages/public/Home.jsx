...
export default function Home() {
  ...
  return <div className="overflow-hidden">
    <section className="relative bg-ink pb-24 pt-20 text-white sm:pt-28 lg:pb-32 lg:pt-36">
      ...
      <div className="absolute left-[10%] top-20 size-80 rounded-full bg-primary/20 blur-[100px]" />
      <div className="container-shell relative">
        <div className="mx-auto max-w-4xl text-center">
          <div className="eyebrow border-primary/25 bg-primary/15 text-primary"><span className="size-1.5 rounded-full bg-primary shadow-[0_0_12px_#7DECE2]" />Your on-demand digital team</div>
          <h1 className="text-5xl font-extrabold leading-[1.02] tracking-[-.055em] sm:text-6xl lg:text-7xl">Grow faster without growing your <span className="brand-gradient-text">headcount.</span></h1>
          ...
          <div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row"><Button to="/pricing" className="px-7 py-4">View plans <ArrowRight size={17} /></Button><Button to="/contact" variant="secondary" className="hero-secondary-button px-7 py-4"><Play size={16} /> Book a free call</Button></div>
          <p className="mt-5 flex items-center justify-center gap-2 text-xs text-slate-400"><CheckCircle2 size={14} className="text-accent" /> Start in 5 days · No long contracts · Cancel anytime</p>
        </div>
      </div>
    </section>
    ...
    <section className="bg-slate-50 py-24 dark:bg-white/[.025] sm:py-32"><div className="container-shell">
      ...
      <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{features.map(({ icon: Icon, title, copy }, index) => <FadeIn key={title} delay={index * .06} className="panel p-6"><span className="grid size-11 place-items-center rounded-xl bg-primary/10 text-electric"><Icon size={21} /></span><h3 className="mt-5 font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{copy}</p></FadeIn>)}</div>
    </div></section>
    ...
    <section className="py-20"><div className="container-shell"><div className="relative overflow-hidden rounded-[2rem] border border-primary/15 bg-ink px-7 py-14 text-center text-slate-100 shadow-2xl shadow-slate-950/20 sm:px-12"><div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_20%,rgba(2,139,127,.24),transparent_34%),radial-gradient(circle_at_88%_12%,rgba(125,236,226,.16),transparent_30%)]" /><div className="absolute -right-20 -top-20 size-72 rounded-full border-[42px] border-primary/10" /><div className="absolute -bottom-24 left-1/2 size-80 -translate-x-1/2 rounded-full bg-primary/10 blur-3xl" /><h2 className="relative text-3xl font-extrabold tracking-tight sm:text-4xl">Your next growth chapter can start this week.</h2><p className="relative mx-auto mt-4 max-w-xl text-slate-300">Tell us where you’re headed. We’ll show you the clearest path there.</p><Button to="/contact" className="relative mt-8 shadow-lg shadow-slate-950/20">Book your free strategy call <ArrowRight size={16} /></Button></div></div></section>
  </div>;
}
