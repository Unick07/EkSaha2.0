...
const formatDate = (value) => value ? new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric" }).format(new Date(value)) : "Not available";
...
export default function Overview() {
  ...
  return <div className="space-y-7">
    <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-5">{kpis.map(([Icon, value, label]) => <div className="panel p-5" key={label}><span className="grid size-11 place-items-center rounded-xl bg-primary/10 text-electric dark:bg-primary/10"><Icon size={20}/></span><div className="mt-5 text-2xl font-extrabold">{value}</div><div className="mt-1 text-sm text-slate-500">{label}</div></div>)}</div>
    <div className="panel p-6"><div><h2 className="font-bold">Database overview</h2><p className="mt-1 text-xs text-slate-500">Live totals from D1.</p></div><div className="mt-7 h-72"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#94a3b822"/><XAxis dataKey="label" axisLine={false} tickLine={false} fontSize={11}/><YAxis axisLine={false} tickLine={false} fontSize={11}/><Tooltip/><Bar dataKey="value" fill="#028B7F" radius={[6, 6, 0, 0]}/></BarChart></ResponsiveContainer></div></div>
