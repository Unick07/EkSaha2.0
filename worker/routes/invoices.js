...
export async function handleInvoices(request, env, path) {
  ...
  if (request.method === "GET" && path === "/invoices") {
    ...
    const rows = await all(env.DB, "SELECT * FROM invoices ORDER BY created_at DESC");
    return json(rows.map(normalizeInvoice), {}, env, request);
  }
  ...
  if (request.method === "POST" && path === "/invoices") {
    ...
    return json(normalizeInvoice(await first(env.DB, "SELECT * FROM invoices WHERE id = ?", [id])), { status: 201 }, env, request);
  }
  ...
  if (request.method === "PATCH" && id) {
    ...
    return json(normalizeInvoice(await first(env.DB, "SELECT * FROM invoices WHERE id = ?", [id])), {}, env, request);
  }
